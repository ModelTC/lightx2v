...
from lightx2v.text2v.models.schedulers.hunyuan.feature_caching.scheduler import (
    HunyuanSchedulerFeatureCaching,
)
from lightx2v.text2v.models.schedulers.wan.scheduler import WanScheduler
from lightx2v.text2v.models.schedulers.wan.feature_caching.scheduler import (
...
from lightx2v.text2v.models.video_encoders.hf.autoencoder_kl_causal_3d.model import (
...
def load_models(args, model_config):
    ...
    if args.model_cls == "hunyuan":
        text_encoder_1 = TextEncoderHFLlamaModel(
            os.path.join(args.model_path, "text_encoder"), init_device
        )
        text_encoder_2 = TextEncoderHFClipModel(
            os.path.join(args.model_path, "text_encoder_2"), init_device
        )
        text_encoders = [text_encoder_1, text_encoder_2]
        model = HunyuanModel(args.model_path, model_config)
        vae_model = VideoEncoderKLCausal3DModel(
            args.model_path, dtype=torch.float16, device=init_device
        )
        ...
        vae_model = WanVAE(
            vae_pth=os.path.join(args.model_path, "Wan2.1_VAE.pth"),
            device=init_device,
            parallel=args.parallel_vae,
        )
        if args.task == "i2v":
            image_encoder = CLIPModel(
                ...
                checkpoint_path=os.path.join(
                    args.model_path, "models_clip_open-clip-xlm-roberta-large-vit-huge-14.pth"
                ),
...
def run_text_encoder(args, text, text_encoders, model_config):
    ...
    if args.model_cls == "hunyuan":
        for i, encoder in enumerate(text_encoders):
            ...
            text_encoder_output[f"text_encoder_{i + 1}_text_states"] = text_state.to(
                dtype=torch.bfloat16
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_cls", type=str, required=True, choices=["wan2.1", "hunyuan"], default="hunyuan"
    )
    ...
    parser.add_argument(
        "--feature_caching", choices=["NoCaching", "TaylorSeer", "Tea"], default="NoCaching"
    )
    ...
    if not args.parallel_attn or (args.parallel_attn and dist.get_rank() == 0):
        if args.model_cls == "wan2.1":
            cache_video(
                tensor=images,
                save_file=args.save_video_path,
                fps=16,
                nrow=1,
                normalize=True,
                value_range=(-1, 1),
