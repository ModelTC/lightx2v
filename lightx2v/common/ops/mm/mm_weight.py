...
class MMWeightWfp8channelAfp8channeldynamicVllm(MMWeightTemplate):
    ...
    def load(self, weight_dict):
        if self.config.get("weight_auto_quant", True):
            ...
            self.weight = weight_dict[self.weight_name].t().cuda()
            self.weight_scale = weight_dict[
                self.weight_name.rstrip(".weight") + ".weight_scale"
            ].cuda()
        self.bias = weight_dict[self.bias_name].cuda() if self.bias_name is not None else None
    ...
    def apply(self, input_tensor):
        ...
        qinput, x_scale = ops.scaled_fp8_quant(
            input_tensor, None, scale_ub=None, use_per_token_if_dynamic=True
        )
        torch.ops._C.cutlass_scaled_mm(
            output_tensor, qinput, self.weight, x_scale, self.weight_scale, self.bias
        )
...
class MMWeightWfp8channelAfp8channeldynamicVllm(MMWeightTemplate):
    ...
    def apply(self, input_tensor):
        ...
        device = input_tensor.device
        output_tensor = torch.empty(shape, dtype=dtype, device=device, requires_grad=False)
        qinput, x_scale, _ = ops.scaled_int8_quant(
            input_tensor, scale=None, azp=None, symmetric=True
        )
        torch.ops._C.cutlass_scaled_mm(
            output_tensor, qinput, self.weight, x_scale, self.weight_scale, self.bias
        )
...
if __name__ == "__main__":
    ...
    mm_weight = MM_WEIGHT_REGISTER["W-fp8-channel-sym-A-fp8-channel-sym-dynamic-Vllm"](
        "xx.weight", "xx.bias"
    )
    ...
    mm_weight = MM_WEIGHT_REGISTER["W-int8-channel-sym-A-int8-channel-sym-dynamic-Vllm"](
        "xx.weight", "xx.bias"
    )
    mm_weight.set_config({"weight_auto_quant": True})
    mm_weight.load(weight_dict)
