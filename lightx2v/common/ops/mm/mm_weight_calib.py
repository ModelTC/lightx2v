...
class MMWeightCalib(MMWeight):
    ...
    def load(self, weight_dict):
        ...
        self.realq_weight = (
            self.realq_weight.view(shape_and_dtype["tensor"][0])
            .contiguous()
            .to(shape_and_dtype["tensor"][1])
        )
        self.scales = (
            self.scales.view(shape_and_dtype["scales"][0])
            .contiguous()
            .to(shape_and_dtype["scales"][1])
        )
        if self.zeros is not None:
            self.zeros = (
                self.zeros.view(shape_and_dtype["zeros"][0])
                .contiguous()
                .to(shape_and_dtype["zeros"][1])
            )
