...
class WanTransformerInfer:
    ...
    def infer_block(self, weights, grid_sizes, embed, x, embed0, seq_lens, freqs, context):
        ...
        q = rms_norm(weights.self_attn_q.apply(norm1_out), weights.self_attn_norm_q_weight, 1e-6).view(s, n, d)

        k = rms_norm(weights.self_attn_k.apply(norm1_out), weights.self_attn_norm_k_weight, 1e-6).view(s, n, d)
        ...
        q = rms_norm(weights.cross_attn_q.apply(norm3_out), weights.cross_attn_norm_q_weight, 1e-6).view(-1, n, d)

        k = rms_norm(weights.cross_attn_k.apply(context), weights.cross_attn_norm_k_weight, 1e-6).view(-1, n, d)

        v = weights.cross_attn_v.apply(context).view(-1, n, d)

        if self.task == "i2v":
            k_img = rms_norm(
                weights.cross_attn_k_img.apply(context_img),
                weights.cross_attn_norm_k_img_weight,
                1e-6,
            ).view(-1, n, d)
