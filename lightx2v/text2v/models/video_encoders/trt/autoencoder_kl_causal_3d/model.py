...
class VideoEncoderKLCausal3DModel:
    ...
    def load(self):
        ...
        ckpt = torch.load(
            os.path.join(self.vae_path, "pytorch_model.pt"), map_location="cpu", weights_only=True
        )
        self.model.load_state_dict(ckpt)
        self.model = self.model.to(dtype=self.dtype, device=self.device)
        self.model.requires_grad_(False)
        self.model.eval()
        trt_decoder = trt_vae_infer.HyVaeTrtModelInfer(
            engine_path=os.path.join(self.vae_path, "vae_decoder.engine")
        )
...
if __name__ == "__main__":
    vae_model = VideoEncoderKLCausal3DModel(
        "/mnt/nvme1/yongyang/models/hy/ckpts", dtype=torch.float16, device=torch.device("cuda")
    )
