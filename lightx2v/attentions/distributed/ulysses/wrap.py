...
def parallelize_hunyuan(hunyuan_model):
    ...
    def new_infer(self, text_encoders_output, image_encoder_output, args):
        ...
        # 预处理输入数据以适应并行计算
        self.scheduler.latents, self.scheduler.freqs_cos, self.scheduler.freqs_sin, split_dim = (
            pre_process(self.scheduler.latents, self.scheduler.freqs_cos, self.scheduler.freqs_sin)
        )
