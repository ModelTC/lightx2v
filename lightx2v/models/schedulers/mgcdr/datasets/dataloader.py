...
try:
    ...
    NuScenesMultiResDataset = None
    NuScenesVariableBatchSampler = None
    NuScenesTDataset = None
    print(f"NuScenesMultiResDataset import failed !!!")
from .pap_variable import PAPMultiResDataset, PAPVariableBatchSampler
...
def prepare_dataloader(
    ...
    **kwargs,
    ...
    _kwargs = kwargs.copy()
    print(f'prepare_dataloader kwargs: {kwargs}')
    # pdb.set_trace()
    if (NuScenesMultiResDataset is not None) and isinstance(dataset, NuScenesMultiResDataset):
    ...
    # elif isinstance(dataset,  (PAPMultiResDataset, Subset)):
    elif isinstance(dataset, PAPMultiResDataset):
        batch_sampler = PAPVariableBatchSampler(
            dataset,
            bucket_config,
            num_replicas=process_group.size(),
            rank=process_group.rank(),
    ...
    elif isinstance(dataset, Subset) or (NuScenesTDataset is not None) and isinstance(dataset, (NuScenesTDataset, Subset)):
