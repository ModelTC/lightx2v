...
class CameraInstance3DBoxes(BaseInstance3DBoxes):
    ...
    def __init__(self, tensor, box_dim=7, with_yaw=True, origin=(0.5, 1.0, 0.5), **kwargs):
...
class RoadCamInstance3DBoxes(CameraInstance3DBoxes):
    def __init__(self, tensor, gplane=None, box_dim=7, with_yaw=True, origin=(0.5, 1.0, 0.5)):
        super(RoadCamInstance3DBoxes, self).__init__(tensor, box_dim, with_yaw, origin)
        assert (
            gplane is not None
        ), "ground-plane is required in  \
                                        roadside camera coordination."
        if isinstance(gplane, torch.Tensor):
            self.gplane = gplane.clone().to(self.tensor.device)
        elif isinstance(gplane, np.ndarray):
            self.gplane = torch.from_numpy(gplane[np.newaxis, :].astype(np.float32)).to(self.tensor.device)
        else:
            self.gplane = torch.as_tensor(gplane, dtype=torch.float32, device=self.tensor.device)
        self.flipped = False

    def gen_rot_from_gplane(self, hflip=False, vflip=False):
        # rotate with ground plane.
        # refer to https://github.com/FlorinShum/Rope3D-Toolkit
        # /blob/main/visualize/vis_utils.py#L330
        a, b, c = -self.gplane[:, 0], -self.gplane[:, 1], -self.gplane[:, 2]
        y_rot = torch.stack([a, b, c], dim=0)
        x_rot = torch.stack([torch.ones_like(a), -a / b, torch.zeros_like(a)])
        div = a**2 + b**2
        z_rot = torch.stack([(-a * c) / div, (-b * c) / div, torch.ones_like(a)])
        if hflip:
            y_rot[0, :] = -y_rot[0, :]
            x_rot[1, :] = -x_rot[1, :]
            z_rot[0, :] = -z_rot[0, :]
            ...
            raise NotImplementedError(
                "Vertical flip is not \
                            supported in RoadCamInstance3DBoxes."
            )
        R_p = torch.stack([x_rot, y_rot, z_rot])
    ...
    def corners(self):
        ...
        corners = rotation_3d_in_axis(corners, self.tensor[:, 6], axis=self.YAW_AXIS)
        # rotate with ground plane.
        R_p = self.gen_rot_from_gplane(hflip=self.flipped)
        # corners (1, 8, 3), R_p (3, 3, 1)
        corners = torch.einsum("aij, jka->aik", corners, R_p.repeat(1, 1, corners.shape[0]))
        # transform with 3d center
        corners += self.tensor[:, :3].view(-1, 1, 3)
        return corners

    def flip(self, bev_direction="horizontal", points=None):
        res = super(RoadCamInstance3DBoxes, self).flip(bev_direction, points)
        self.flipped = True
        if points is not None:
            return res
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, int):
            return original_type(self.tensor[item].view(1, -1), self.gplane, box_dim=self.box_dim, with_yaw=self.with_yaw)
        b = self.tensor[item]
        assert b.dim() == 2, f"Indexing on Boxes with {item} failed to return a matrix!"
        return original_type(b, self.gplane, box_dim=self.box_dim, with_yaw=self.with_yaw)

    def to(self, device):
        original_type = type(self)
        return original_type(self.tensor.to(device), self.gplane.to(device), box_dim=self.box_dim, with_yaw=self.with_yaw)

    def clone(self):
        original_type = type(self)
        return original_type(self.tensor.clone(), self.gplane.clone(), box_dim=self.box_dim, with_yaw=self.with_yaw)

    def new_box(self, data, gplane):
        new_tensor = self.tensor.new_tensor(data) if not isinstance(data, torch.Tensor) else data.to(self.device)

        gplane = self.gplane.new_tensor(gplane) if not isinstance(gplane, torch.Tensor) else gplane.to(self.device)

        original_type = type(self)
        return original_type(new_tensor, gplane, box_dim=self.box_dim, with_yaw=self.with_yaw)
    ...
    def convert_to(self, dst, rt_mat=None):
        ...
        return Box3DMode.convert(box=self, src=Box3DMode.ROADCAM, dst=dst, rt_mat=rt_mat)
