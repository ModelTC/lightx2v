...
# Import from alt
from alt.coordinate.utils import array_converter
...
def get_box_type(box_type):
    ...
    elif box_type_lower == "roadcam":
        box_type_3d = RoadCamInstance3DBoxes
        box_mode_3d = Box3DMode.ROADCAM
    else:
...
def points_cam2img(points_3d, proj_mat, with_depth=False, clamp=False):
    ...
    assert len(proj_mat.shape) == 2, "The dimension of the projection" f" matrix should be 2 instead of {len(proj_mat.shape)}."
    ...
    if clamp:
        point_2d[..., 2:3] = torch.clamp(point_2d[..., 2:3], min=1e-5, max=1e5)
...
def get_lidar2img(cam2img: torch.Tensor, lidar2cam: torch.Tensor) -> torch.Tensor:
