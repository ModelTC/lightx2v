from .base_hunyuan_transformer_inefr import BaseHunyuanTransformer
import torch
import numpy as np

class HunyuanTransformerTaylorCaching(BaseHunyuanTransformer):
    ...
    def infer_calculating(self, weights, img, txt, vec, cu_seqlens_qkv, max_seqlen_qkv, freqs_cis, token_replace_vec=None, frist_frame_token_num=None):
        ...
        for i in range(self.double_blocks_num):
            ...
            self.double_blocks_cache[i]["img_attn"] = img_out
            self.double_blocks_cache[i]["txt_attn"] = txt_out
            img_out, txt_out, img_mod2_gate, txt_mod2_gate = super().infer_double_block_2(weights.double_blocks[i], img, txt, vec, cu_seqlens_qkv, max_seqlen_qkv, freqs_cis, token_replace_vec, frist_frame_token_num, img_out, txt_out, img_mod1_gate, img_mod2_shift, img_mod2_scale, img_mod2_gate,tr_img_mod1_gate, tr_img_mod2_shift, tr_img_mod2_scale, tr_img_mod2_gate, txt_mod1_gate, txt_mod2_shift, txt_mod2_scale, txt_mod2_gate)
            self.double_blocks_cache[i]["img_mlp"] = img_out
            self.double_blocks_cache[i]["txt_mlp"] = txt_out
            img, txt = super().infer_double_block_3(self, weights, img, txt, vec, cu_seqlens_qkv, max_seqlen_qkv, freqs_cis, token_replace_vec, frist_frame_token_num, img_out, txt_out, img_mod2_gate, txt_mod2_gate)

        x = torch.cat((img, txt), 0)
        for i in range(self.single_blocks_num):
            out, mod_gate = super().infer_single_block_1(weights.single_blocks[i], x, vec, txt_seq_len, cu_seqlens_qkv, max_seqlen_qkv, freqs_cis, token_replace_vec, frist_frame_token_num)
            self.single_blocks_cache[i]["total"] = out
            x = super().infer_single_block_2(weights.single_blocks[i], x, vec, txt_seq_len, cu_seqlens_qkv, max_seqlen_qkv, freqs_cis, out, mod_gate, token_replace_vec, frist_frame_token_num)
        img = x[:img_seq_len, ...]
        return img, vec
    ...
    def infer_double_block(self, weights, img, txt, vec, cu_seqlens_qkv, max_seqlen_qkv, freqs_cis, i):
        ...
        out = self.taylor_formula(self.double_blocks_cache[i]["img_attn"], self.scheduler)
        out = out * img_mod1_gate
        img = img + out

        out = self.taylor_formula(self.double_blocks_cache[i]["img_mlp"], self.scheduler)
        out = out * img_mod2_gate
        img = img + out

        out = self.taylor_formula(self.double_blocks_cache[i]["txt_attn"], self.scheduler)
        out = out * txt_mod1_gate
        txt = txt + out

        out = self.taylor_formula(self.double_blocks_cache[i]["txt_mlp"], self.scheduler)
    ...
    def infer_single_block(self, weights, x, vec, txt_seq_len, cu_seqlens_qkv, max_seqlen_qkv, freqs_cis, i):
        ...
        out = self.taylor_formula(self.single_blocks_cache[i]["total"], self.scheduler)
        out = out * mod_gate
        x = x + out
        return x
